...
class HiddenWriteReviewActivity : AppCompatActivity(), HTTPConnectionListener, View.OnClickListener {
    ...
    private val UPLOAD_HIDDEN_REVIEW = 1
            ...
            R.id.writeButton -> {
                ...
                if (review.trim() != "") {
                    val postData = JSONObject().apply {
                        put("what", "uploadHiddenAreaReview")
                        put("id", itemId)
                        put("review", reviewET.text.toString())
                        put("star", reviewRB.rating)
                    }
                    httpConnectionService.serverPostRequest(serverPage, postData.toString(), HTTPConnectionThread.REQUEST_SIMPLE_POST_METHOD, UPLOAD_HIDDEN_REVIEW)
                } else {
    ...
    private val connection: ServiceConnection = object : ServiceConnection {
        override fun onServiceConnected(name: ComponentName, service: IBinder) {
            ...
            httpConnectionService.setOnHttpRespondListener(this@HiddenWriteReviewActivity)
        }
